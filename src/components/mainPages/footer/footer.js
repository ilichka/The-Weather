...
import { renderPageById } from "../../../utils/utils";

const addFooterListener = (routes) => {
  document.querySelectorAll(".footer .icon").forEach((icon, index) => {
    icon.addEventListener("click", () => {
      location.hash = routes[index].route;
    });
  });
};

const appendFooter = (routes) => {
  ...
  renderPageById("app", footer);
  addFooterListener(routes);
};
